...
#Numero de Copias
incCopys=`cd /copias_tar | ls | grep copiaIncremental_*.tar | wc -l`
...
weekInc(){
    ...
    echo "--------------------------------------------------------------"
    tar cvzfP /copias_tar/incremental/copiaIncremental_$date2\_$numCopy.tar -g /copias_tar/incremental/history/backup.snap $HOME/*
    echo "--------------------------------------------------------------"
}
